...
{
	public class PROVIDER_DEBT:Object
	{
		#region Init
		private string m_ID;
			...
			bool Active)
		{
			...
			"@Sorted",
			"@Active"};
			object[] myValue={ID,RefID,RefDate,RefOrgNo,RefType,RefStatus,PaymentMethod,CustomerID,ProductID,ProductName,CurrencyID,ExchangeRate,TermID,DueDate,Quantity,ReQuantity,Amount,Payment,Balance,FAmount,FPayment,FBalance,Discount,FDiscount,IsChanged,IsPublic,CreatedBy,CreatedDate,ModifiedBy,ModifiedDate,OwnerID,Description,Sorted,Active};
			...
			bool Active)
		{
			...
			SqlHelper mySql=new SqlHelper();
			mySql.Error += DispError;
			return mySql.ExecuteNonQuery(myConnection,"PROVIDER_DEBT_UpdateByID",myPara,myValue)=="OK"?true:false;
		}
		public bool UpdateByID(
			SqlTransaction myTransaction,
			string ID,
			...
			bool Active)
		{
			...
			SqlHelper mySql=new SqlHelper();
			mySql.Error += DispError;
			return mySql.ExecuteNonQuery(myTransaction,"PROVIDER_DEBT_UpdateByID",myPara,myValue)=="OK"?true:false;
		}
		#endregion
	...
	public enum PROVIDER_DEBTFields
		{
			ID,
			RefID,
			RefDate,
	...
	public class PROVIDER_DEBTCollection : System.Collections.CollectionBase
		{
			[Description("Adds a new PROVIDER_DEBT to the collection.")]
			public int Add(PROVIDER_DEBT item)
				{
					int newindex = List.Add(item);
					return newindex;
				}
			[Description("Removes a PROVIDER_DEBT from the collection.")]
			public void Remove(PROVIDER_DEBT item)
				{
					List.Remove(item);
				}
			[Description("Inserts an PROVIDER_DEBT into the collection at the specified index..")]
			public void Inserts(int index,PROVIDER_DEBT item)
				{
					List.Insert(index,item);
				}
			[Description("Returns the index value of the BRANCH class in the collection.")]
			public int IndexOf(PROVIDER_DEBT item)
				{
					return List.IndexOf(item);
				}
			[Description("Returns true if the PROVIDER_DEBT class is present in the collection.")]
			public bool Contains(PROVIDER_DEBT item)
				{
