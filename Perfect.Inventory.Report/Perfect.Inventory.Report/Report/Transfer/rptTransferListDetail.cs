...
{
    public partial class rptTransferListDetail : XtraReport
    {
        public rptTransferListDetail(DataTable dt, string ID,string title, string Comment)
        {
            InitializeComponent();
            ...
            if (dt != null)
            {
                if (dt.Rows.Count != 0)
                {
                    dsTRANSFER1.STOCK_TRANSFER_DETAIL.Merge(dt);
                }
            }
