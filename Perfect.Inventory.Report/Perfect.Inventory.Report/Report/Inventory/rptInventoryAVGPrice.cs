...
{
    public partial class rptInventoryAVGPrice : XtraReport
    {
        int Count = 0;
        

        public rptInventoryAVGPrice(int action, string id, DataTable dt, string title, string comment, string filterstring)
        {
            InitializeComponent();
            ...
            if (dt != null)
            {
                if (dt.Rows.Count != 0)
                {
                    ...
                    dsInventory1.INVENTORY_AVGPRICE_DYNAMIC.Merge(dt);
                }
            }
